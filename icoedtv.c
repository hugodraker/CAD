/* ============================================================================
 * Vector Icon Editor for Win32 - Saves icons as GDI Polygons
 * ============================================================================
 * PORTED TO WIN32 GCC (MinGW)
 * gcc -Os -s -std=c99 -mwindows icoedtv.c -o icoedtv.exe -lcomdlg32 -lshell32
 *
        ...
        if (f) {
            ...
            if (iconShapes) {
                for (j = 0; j < iconShapeCount; j++) {
                    if (iconShapes[j].type == 3) continue;
...
int EnsureRefLoaded(const char* path) {
    ...
    for (i = 0; i < refCacheCount; i++) {
        if (_stricmp(refCache[i].path, path) == 0) return i;
    }
    if (refCacheCount >= MAX_REFS) return -1;

    if (!refCache[refCacheCount].shapes) {
        refCache[refCacheCount].shapes = (Shape*)calloc(1, MAX_SHAPES * sizeof(Shape));
...
void GetRelativePath(const char* base, const char* target, char* out) {
    ...
    while (i < bCount && i < tCount && _stricmp(bDirs[i], tDirs[i]) == 0) i++;
...
int IsRefFile(const char* path) {
    int i;
    for (i = 0; i < refCacheCount; i++) {
        if (_stricmp(refCache[i].path, path) == 0) return 1;
    }
    return 0;
}

LRESULT CALLBACK MultiEditProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    return CallWindowProc((WNDPROC)oldMultiEditProc, hwnd, msg, wParam, lParam);
}
...
LRESULT CALLBACK PageSizeProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
...
LRESULT CALLBACK RefListProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
...
LRESULT CALLBACK ScaleDlgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
        case WM_CREATE:
...
LRESULT CALLBACK TagEditorProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
...
void CommitCurrentIcon(void) {
    ...
    if (currentIconIdx >= 0 && currentIconIdx < parsedCount) {
        ...
        if (shapeCount > 0) {
            parsedIcons[currentIconIdx].shapes = (Shape*)calloc(1, sizeof(Shape) * shapeCount);
...
void LoadCFile(const char* path, HWND hwnd) {
    ...
    if (d) {
        ...
        while ((cur = strstr(cur, "case ")) != NULL && parsedCount < MAX_ICONS) {
            ...
            if (tmpCount > 0) {
                exact = (Shape*)calloc(1, tmpCount * sizeof(Shape));
...
void DoSaveFile(HWND hwnd) {
    ...
    if (loadedCFile[0] != '\0') {
        for (i = 0; i < refCacheCount; i++) {
            if (_stricmp(refCache[i].path, loadedCFile) == 0) {
...
void AddEdgesFromShape(Shape* s1, Shape* s2, Edge* pool, int* edgeCount) {
    ...
    double* cutsX = (double*)calloc(1, MAX_POINTS * sizeof(double));
    double* cutsY = (double*)calloc(1, MAX_POINTS * sizeof(double));
    int cutCount = 0;
...
void DrawGrid(HDC dc) {
    ...
    for (i = 0; i <= gridH; i++) { 
        MoveToEx(dc, (int)viewPanX, (int)(viewPanY + i * sc), NULL); 
        LineTo(dc, (int)(viewPanX + canvasW_px * viewZoom), (int)(viewPanY + i * sc)); 
    }
    for (i = 0; i <= gridW; i++) { 
        MoveToEx(dc, (int)(viewPanX + i * sc), (int)viewPanY, NULL); 
        ...
        for (i = 0; i <= sCnt; i++) {
            ...
            if (s->type == 3) {
...

                {
                    ...
                    if (loadedCFile[0] && _stricmp(absPath, loadedCFile) == 0) continue;
                            ...
                            for (dIdx = 0; dIdx < refCache[rIdx].dimCount; dIdx++) {
                                ...
                                if (d->s1 < refCache[rIdx].shapeCount && d->s2 < refCache[rIdx].shapeCount) {
...

                                    {
                                        ...
                                        if (L1len > 5.0) { MoveToEx(dc, sA1x + (int)round(L1dx/L1len*5.0), sA1y + (int)round(L1dy/L1len*5.0), NULL); LineTo(dc, sD1x + (int)round(L1dx/L1len*2.0), sD1y + (int)round(L1dy/L1len*2.0)); }
                                        double L2dx = sD2x - sA2x, L2dy = sD2y - sA2y, L2len = sqrt(L2dx*L2dx + L2dy*L2dy);
                                        if (L2len > 5.0) { MoveToEx(dc, sA2x + (int)round(L2dx/L2len*5.0), sA2y + (int)round(L2dy/L2len*5.0), NULL); LineTo(dc, sD2x + (int)round(L2dx/L2len*2.0), sD2y + (int)round(L2dy/L2len*2.0)); }
                                    }
                                    MoveToEx(dc, sD1x, sD1y, NULL); LineTo(dc, sD2x, sD2y);
                                    {
                                    ...
                                    SIZE ext; GetTextExtentPoint32(dc, buf, strlen(buf), &ext); int tW = ext.cx, tH = ext.cy, tx = sMidX - tW/2, ty = sMidY - tH/2;
...
void DrawDimensions(HDC dc) {
    ...
    for (i = 0; i < dimCount; i++) {
...

        {
            ...
            if (L1len > 5.0) {
                MoveToEx(dc, (int)(A1x + L1dx/L1len*5.0), (int)(A1y + L1dy/L1len*5.0), NULL);
            ...
            if (L2len > 5.0) {
                MoveToEx(dc, (int)(A2x + L2dx/L2len*5.0), (int)(A2y + L2dy/L2len*5.0), NULL);
                LineTo(dc, (int)(D2x + L2dx/L2len*2.0), (int)(D2y + L2dy/L2len*2.0));
            }
        }

        MoveToEx(dc, (int)D1x, (int)D1y, NULL); LineTo(dc, (int)D2x, (int)D2y);
...

        {
            ...
            GetTextExtentPoint32(dc, buf, strlen(buf), &ext);
            tW = ext.cx;
            tH = ext.cy;
            tx = (int)(midX - tW/2.0);
            ty = (int)(midY - tH/2.0);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (msg) {
        case WM_CREATE: {
            ...
            shapes = (Shape*)calloc(1, (DWORD)MAX_SHAPES * sizeof(Shape));
            dragStartSnapshot = (Shape*)calloc(1, (DWORD)MAX_SHAPES * sizeof(Shape));
            
            g_tempRef = (Shape*)calloc(1, 5 * sizeof(Shape));
            g_subPts = (POINT*)calloc(1, 5 * MAX_POINTS * sizeof(POINT));
            g_pA = (POINT*)calloc(1, 5 * MAX_POINTS * sizeof(POINT));

            for(i=0; i<MAX_UNDO; i++) {
                history[i] = (Shape*)calloc(1, (DWORD)MAX_SHAPES * sizeof(Shape));
                if (!history[i]) allocFailed = 1;
            }
            
            parsedIcons = (IconDef*)calloc(1, (DWORD)MAX_ICONS * sizeof(IconDef));
            refCache = (RefCache*)calloc(1, (DWORD)MAX_REFS * sizeof(RefCache));
            editMap = (TagEditMap*)calloc(1, (DWORD)MAX_EDIT_TAGS * sizeof(TagEditMap));

            if (!shapes || !dragStartSnapshot || allocFailed || !parsedIcons || !refCache || !editMap || !g_tempRef || !g_subPts || !g_pA) {
                MessageBox(hwnd, "Failed to allocate memory on Heap!", "Error", MB_ICONHAND);
            ...
            oldMultiEditProc = (FARPROC)SetWindowLongPtr(hMultiEdit, GWLP_WNDPROC, (LONG_PTR)MultiEditProc);
            break;
        }
        case WM_DROPFILES: {
            char filePath[260];
        ...
        case WM_HSCROLL: {
            HWND hTrk = (HWND)(UINT_PTR)lParam; UINT code = LOWORD(wParam); int pos, p;
            ...
            if (hTrk == hScrlIcon) {
                ...
                if (code == SB_THUMBTRACK || code == SB_THUMBPOSITION) { pos = (int)(short)HIWORD(wParam); } 
        ...
        case WM_LBUTTONDOWN: {
            ...
            if (dimCount > 0) {
                ...
                for (i = 0; i < dimCount; i++) {
                    ...
                    double midX, midY, val; char buf[64]; SIZE ext; int tW, tH, tx, ty, hx, hy;
                    double scOffset;
                    ...
                    GetTextExtentPoint32(hdc, buf, strlen(buf), &ext);
                    tW = ext.cx; tH = ext.cy; tx = (int)(midX - tW/2.0); ty = (int)(midY - tH/2.0);
        ...
        case WM_COMMAND: {
            ...
            if (id == 151) { 
                ...
                if (parsedCount > 0 && currentIconIdx >= 0) {
                    if (parsedIcons[currentIconIdx].shapes) {
                        free(parsedIcons[currentIconIdx].shapes);
                        parsedIcons[currentIconIdx].shapes = NULL;
                    }
            ...
            if (btnId >= 0 && btnId < 33) {
                if (btnId >= 0 && btnId <= 5) { 
                    ...
                    ofn.lpstrFile = szFile; ofn.nMaxFile = sizeof(szFile); ofn.Flags = OFN_FILEMUSTEXIST;
                    if (GetOpenFileName(&ofn)) {
                        if (loadedCFile[0] && _stricmp(szFile, loadedCFile) == 0) {
                    ...
                    if (distinctCount == 2 && shapes[distinctShapes[0]].type == 0 && shapes[distinctShapes[1]].type == 0) {
                        ...
                        filtered = (Edge*)calloc(1, 1024 * sizeof(Edge));
                        keep = (int*)calloc(1, 1024 * sizeof(int));
                        used = (int*)calloc(1, 1024 * sizeof(int));
                        ...
                        if (clnX) free(clnX); if (clnY) free(clnY);
                        UpdateStatusBar(); RedrawCanvas(hwnd);
                    }
                }
                else if (btnId == 18) { 
        ...
        case WM_DESTROY: {
            int i;
            if (IsWindow(hDistEdit) && oldEditProc) { SetWindowLongPtr(hDistEdit, GWLP_WNDPROC, (LONG_PTR)oldEditProc); oldEditProc = NULL; }
            if (IsWindow(hMultiEdit) && oldMultiEditProc) { SetWindowLongPtr(hMultiEdit, GWLP_WNDPROC, (LONG_PTR)oldMultiEditProc); oldMultiEditProc = NULL; }

            if (shapes) { free(shapes); shapes = NULL; }
            if (dragStartSnapshot) { free(dragStartSnapshot); dragStartSnapshot = NULL; }
            
            if (g_tempRef) { free(g_tempRef); g_tempRef = NULL; }
            if (g_subPts) { free(g_subPts); g_subPts = NULL; }
            if (g_pA) { free(g_pA); g_pA = NULL; }
            ...
            if (parsedIcons) {
                ...
                free(parsedIcons); parsedIcons = NULL;
            }
            if (refCache) {
                for(i=0; i<MAX_REFS; i++) if (refCache[i].shapes) { free(refCache[i].shapes); refCache[i].shapes = NULL; }
                free(refCache); refCache = NULL;
            }
            if (editMap) { free(editMap); editMap = NULL; }
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int nCmdShow) {
    ...
    /* Variables not used in Win32, kept to prevent unused parameter warnings */
    (void)hPrevInstance;
    (void)lpszCmdLine;
    ...
    hMain = CreateWindow("IconEditClass", "Win32 GCC Vector Editor", WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN, CW_USEDEFAULT, CW_USEDEFAULT, 680, 560, NULL, NULL, hInstance, NULL);
