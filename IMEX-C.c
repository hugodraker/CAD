...
/* --- Forward Declarations to satisfy OpenWatcom --- */
...
void DrawDXFDimensions(FILE* f, Dimension* dims, int dimCount, Shape FAR* FAR* shapes, int shapeCount, double pageScale, double offX, double offY, double minX, double minY, const char* unitName);
void DrawSVGDimensions(FILE* f, Dimension* dims, int dimCount, Shape FAR* FAR* shapes, int shapeCount, double pageScale, double offX, double offY, double minX, double minY, const char* unitName);
...
void GetShapePoint(Shape FAR* sh, int ptIdx, double* outX, double* outY) {
    if (sh->type == 3) {
        ...
        if (loadedCFile[0] && stricmp(absPath, loadedCFile) == 0) { *outX = objCx; *outY = objCy; return; }
...
int LoadCFile(const char* path) {
    ...
    Shape FAR** tmpShapesArr; int i, cId = 1, tmpCount = 0;
    ...
    FreeAllData();
    tmpShapesArr = (Shape FAR**)GlobalAllocPtr(GHND, MAX_SHAPES * sizeof(Shape FAR*));
    if (!tmpShapesArr) return 0;
    memset(tmpShapesArr, 0, MAX_SHAPES * sizeof(Shape FAR*));

    strcpy(loadedCFile, path);
    f = fopen(path, "rb"); if (!f) { loadedCFile[0] = '\0'; GlobalFreePtr(tmpShapesArr); return 0; }
    fseek(f, 0, SEEK_END); sz = ftell(f); fseek(f, 0, SEEK_SET);
    if (sz <= 0 || sz > 60000L) { fclose(f); GlobalFreePtr(tmpShapesArr); return 0; }

    d = (char*)GlobalAllocPtr(GHND, (size_t)sz + 1);
    if (!d) { fclose(f); GlobalFreePtr(tmpShapesArr); return 0; }
    fread(d, 1, (size_t)sz, f); d[sz] = '\0'; fclose(f);
