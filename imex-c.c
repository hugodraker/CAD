/* ============================================================================
 * IMEX-C SVG/DXF to C and C to SVG/DXF Importer/Exporter - Win32 GCC
 *
 * COMPILATION INSTRUCTIONS (GCC/MinGW-w64):
 *   gcc -std=c99 -Os -s -mwindows IMEX-C.c -o imex-c.exe -lcomdlg32 -lshell32
 *
 * REQUIREMENTS: Windows XP or later (Win32 API)
 * DEPENDENCIES: USER32, GDI32, COMDLG32, SHELL32
 *
...
int parsedCount = 0;
RefCache* refCache[MAX_REFS];
int refCacheCount = 0;
char loadedCFile[260] = "";
int g_RenderRefDepth = 0;

Shape* tempShapes[MAX_SHAPES];
int tempShapeCount = 0;
Dimension tempDims[MAX_DIMS];
int tempDimCount = 0;

/* --- Forward Declarations --- */
void GetShapePoint(Shape* sh, int ptIdx, double* outX, double* outY);
void FormatDimension(double val, const char* unit, char* outBuf);
void ParseExtRefString(const char* text, char* refPath, double* rScale, double* rRot);
...
void DrawDXFDimensions(FILE* f, Dimension* dims, int dimCount, Shape** shapes, int shapeCount, double pageScale, double offX, double offY, double minX, double minY, const char* unitName);
void DrawSVGDimensions(FILE* f, Dimension* dims, int dimCount, Shape** shapes, int shapeCount, double pageScale, double offX, double offY, double minX, double minY, const char* unitName);
void DrawDXFShape(FILE* f, Shape* sh, double scale, double iconPageScale);
void DrawSVGShape(FILE* f, Shape* sh, double scale, double minX, double minY, double iconPageScale);
...
void DrawDXFDimensions(FILE* f, Dimension* dims, int dimCount, Shape** shapes, int shapeCount, double pageScale, double offX, double offY, double minX, double minY, const char* unitName) {
    ...
    for (i = 0; i < dimCount; i++) {
        ...
        double lDx, lDy, val, lGridOffset;
        char buf[64];
        ...
        if (dims[i].mode == 0) {
            double ang = (lDx == 0.0 && lDy == 0.0) ? 0.0 : atan2(lDy, lDx);
        ...
        double lMidX = lD1x + (lD2x - lD1x) * dims[i].textPos;
        double lMidY = lD1y + (lD2y - lD1y) * dims[i].textPos;

        double svgA1x = offX + (lA1x - minX) * drawScale; double svgA1y = offY + (lA1y - minY) * drawScale;
        double svgA2x = offX + (lA2x - minX) * drawScale; double svgA2y = offY + (lA2y - minY) * drawScale;
        double svgD1x = offX + (lD1x - minX) * drawScale; double svgD1y = offY + (lD1y - minY) * drawScale;
        double svgD2x = offX + (lD2x - minX) * drawScale; double svgD2y = offY + (lD2y - minY) * drawScale;
        double svgMidX = offX + (lMidX - minX) * drawScale; double svgMidY = offY + (lMidY - minY) * drawScale;
...
void DrawDXFShape(FILE* f, Shape* sh, double scale, double iconPageScale) {
    ...
    if (sh->type == 3) {
        ...
        double rScale = 1.0, rRot = 0.0;

        ParseExtRefString(sh->text, refPath, &rScale, &rRot);
        if (!refPath[0]) return;
        ...
        if (rIdx != -1) {
            double objCx = sh->ptsX[0];
            double objCy = sh->ptsY[0];
            ...
            for (r = 0; r < refCache[rIdx]->shapeCount; r++) {
                ...
                if (sub->type == 3) {
                    if (g_RenderRefDepth < 3) {
                        Shape* tRef = (Shape*)calloc(1, sizeof(Shape));
                        char* subPath = subPath_stack[g_RenderRefDepth];

                        if (tRef) {
                            double subSc = 1.0, subRot = 0.0;
                            ParseExtRefString(sub->text, subPath, &subSc, &subRot);
                            
                            *tRef = *sub;
                            double dx = sub->ptsX[0] * rScale, dy = sub->ptsY[0] * rScale;
                            tRef->ptsX[0] = objCx + dx * cosR - dy * sinR; 
                            tRef->ptsY[0] = objCy + dx * sinR + dy * cosR;
                    ...
                    if (tmpShp) {
                        ...
                        *tmpShp = *sub;
                        for (pIdx = 0; pIdx < sub->ptCount; pIdx++) {
                            double dx = sub->ptsX[pIdx] * rScale, dy = sub->ptsY[pIdx] * rScale;
                            tmpShp->ptsX[pIdx] = objCx + dx * cosR - dy * sinR; 
                            tmpShp->ptsY[pIdx] = objCy + dx * sinR + dy * cosR;
                        }
            ...
            if (refCache[rIdx]->dimCount > 0) {
                DrawDXFDimensions(f, refCache[rIdx]->dims, refCache[rIdx]->dimCount, refCache[rIdx]->shapes, refCache[rIdx]->shapeCount, scale, 0.0, 0.0, 0.0, 0.0, refCache[rIdx]->unitName);
            }
        } else {
            double fSize = (24.0 * scale) / 10.0;
            fprintf(f, "  0\nTEXT\n  8\n0\n 10\n%f\n 20\n%f\n 30\n0.0\n 40\n%f\n  1\nMissing: %s\n", 
                sh->ptsX[0]*scale, -(sh->ptsY[0]*scale + fSize*0.8), fSize, refPath);
        }
        return;
    }
...
void GetShapePoint(Shape* sh, int ptIdx, double* outX, double* outY) {
    if (sh->type == 3) {
        ...
        double objCx = sh->ptsX[0], objCy = sh->ptsY[0];
        
        if (ptIdx == 0) { *outX = objCx; *outY = objCy; return; }
        
        ParseExtRefString(sh->text, refPath, &rScale, &rRot);
        if (!refPath[0]) { *outX = objCx; *outY = objCy; return; }
        
        char absPath[260];
        ResolvePath(loadedCFile, refPath, absPath);
        /* Prevent infinite loops, but don't abort if the file is simply unsaved */
        if (loadedCFile[0] && _stricmp(absPath, loadedCFile) == 0) { *outX = objCx; *outY = objCy; return; }
        ...
        if (rIdx != -1) {
            ...
            if (ptIdx >= 1 && ptIdx <= 4) {
                ...
                double dx_r = (bx[ptIdx - 1] - lcx) * rScale;
                double dy_r = (by[ptIdx - 1] - lcy) * rScale;
                *outX = objCx + dx_r * cosR - dy_r * sinR;
                *outY = objCy + dx_r * sinR + dy_r * cosR;
                return;
            }
            if (ptIdx == 5) { *outX = objCx; *outY = objCy; return; }
            ...
            for (r = 0; r < refCache[rIdx]->shapeCount; r++) {
                ...
                if (sub->type == 4) {
                    if (currentPt < MAX_POINTS) {
                        if (ptIdx == currentPt) {
                            double dx_r = (sub->ptsX[0] - lcx) * rScale, dy_r = (sub->ptsY[0] - lcy) * rScale;
                            *outX = objCx + dx_r * cosR - dy_r * sinR;
                            *outY = objCy + dx_r * sinR + dy_r * cosR;
                            return;
                        }
                    ...
                    for (pIdxLoop = 0; pIdxLoop < subCount; pIdxLoop++) {
                        if (currentPt < MAX_POINTS) {
                            if (ptIdx == currentPt) {
                                double dx_r = (sub->ptsX[pIdxLoop] - lcx) * rScale, dy_r = (sub->ptsY[pIdxLoop] - lcy) * rScale;
                                *outX = objCx + dx_r * cosR - dy_r * sinR;
                                *outY = objCy + dx_r * sinR + dy_r * cosR;
                                return;
                            }
        ...
        *outX = objCx; *outY = objCy;
    } else {
        if (ptIdx >= 0 && ptIdx < sh->ptCount) {
            *outX = sh->ptsX[ptIdx]; *outY = sh->ptsY[ptIdx];
        } else {
            *outX = sh->ptsX[0]; *outY = sh->ptsY[0];
        }
    }
}
double local_round(double val) { return floor(val + 0.5); }
void ParseExtRefString(const char* text, char* refPath, double* rScale, double* rRot) {
    *rScale = 1.0; *rRot = 0.0; refPath[0] = '\0';
    if (!text) return;
    if (strncmp(text, "{{EXT_REF=", 10) == 0) {
        char *pScale = strstr(text, " scale="); 
        char *pRot = strstr(text, " rot="); 
        char *pEnd = strstr(text, "}}");
        int pathLen;
        if (pScale) pathLen = (int)(pScale - (text + 10)); 
        else if (pRot) pathLen = (int)(pRot - (text + 10)); 
        else if (pEnd) pathLen = (int)(pEnd - (text + 10)); 
        else pathLen = strlen(text + 10);
        
        if (pathLen > 0 && pathLen < 512) {
            strncpy(refPath, text + 10, pathLen); refPath[pathLen] = '\0';
        ...
        strncpy(refPath, text, 511); refPath[511] = '\0';
    }
}
...
void GetPipeValue(const char* pipeStr, int idx, char* out, int maxLen) {
    int curr = 0; const char* start = pipeStr; const char* p = pipeStr; out[0] = '\0';
    if (!pipeStr) return;
    while (*p) {
...
void FormatDimension(double val, const char* unit, char* outBuf) {
    if (_stricmp(unit, "feet-inches") == 0) {
        ...
        if (fabs(inches - local_round(inches)) < 0.001) {
            sprintf(outBuf, "%d'-%d\"", feet, (int)local_round(inches));
        } else if (fabs(inches * 10.0 - local_round(inches * 10.0)) < 0.001) {
            sprintf(outBuf, "%d'-%.1f\"", feet, inches);
        } else {
            sprintf(outBuf, "%d'-%.2f\"", feet, inches);
        }
    } else if (_stricmp(unit, "inches") == 0) {
        if (fabs(val - local_round(val)) < 0.001) sprintf(outBuf, "%d\"", (int)local_round(val));
        else sprintf(outBuf, "%.2f\"", val);
    } else if (_stricmp(unit, "None") == 0 || unit[0] == '\0') {
        if (fabs(val - local_round(val)) < 0.001) sprintf(outBuf, "%d", (int)local_round(val));
        else sprintf(outBuf, "%.2f", val);
    } else {
        if (fabs(val - local_round(val)) < 0.001) sprintf(outBuf, "%d %s", (int)local_round(val), unit);
...
void SilentLoadC(const char* path, RefCache* ref) {
    ...
    ref->shapeCount = 0; ref->dimCount = 0; strcpy(ref->unitName, "mm");
    ...
    if (cur) {
        ...
        while (st < endBlock && ref->shapeCount < MAX_SHAPES) {
            ...
            if (next == pgDef) {
                ...
                char* q[6]; int q_cnt = 0; char* pC = pOpen;
                while (pC && q_cnt < 6) {
            ...
            if (next == dim) { 
                ...
                if (pOpen) {
                    ...
                    if (c1 && c2 && c3 && c4 && c5 && c6 && ref->dimCount < MAX_DIMS) {
                        ref->dims[ref->dimCount].s1 = atoi(pOpen + 1); ref->dims[ref->dimCount].p1 = atoi(c1 + 1);
                        ref->dims[ref->dimCount].s2 = atoi(c2 + 1); ref->dims[ref->dimCount].p2 = atoi(c3 + 1);
                        ref->dims[ref->dimCount].offset = atof(c4 + 1); ref->dims[ref->dimCount].textPos = atof(c5 + 1);
                        ref->dims[ref->dimCount].mode = atoi(c6 + 1); ref->dimCount++;
                    }
                }
                st = strchr(next, ';'); if (!st) st = next + 1; continue; 
            }
            ...
            /* Enable fill and stroke by default so SVG geometries are never invisible */
...

            {
                ...
                if (brushSearch) {
                    ...
                    if (rgb) {
                        ...
                        if (comma1) {
                            ...
                            if (comma2) { int cb = atoi(comma2 + 1); s->fill = RGB(cr, cg, cb); }
                        }
                    }
                }
                ...
                if (penSearch) {
                    ...
                    if (rgb) {
                        ...
                        if (comma1) {
                            int cg = atoi(comma1 + 1); char* comma2 = strchr(comma1 + 1, ',');
                            if (comma2) { int cb = atoi(comma2 + 1); s->stroke = RGB(cr, cg, cb); s->strokeWidth = sw > 0 ? sw : 1; }
                        }
                    }
                }
                *next = savedChar;
            ...
            if (next == pt) {
                ...
                if (pOpen) {
                    ...
                    if (c1 && c2 && c3) {
                        s->ptsX[0] = atof(pOpen + 1); s->ptsY[0] = atof(c1 + 1);
                        s->ptsX[1] = atof(c2 + 1); s->ptsY[1] = atof(c3 + 1);
                ...
                if (pOpen && q1 && q2) {
                    ...
                    s->ptsX[0] = atof(pOpen + 1); s->ptsY[0] = comma1 ? atof(comma1 + 1) : 0.0;
                ...
                char* pOpen = strchr(next, '('); char* q1 = pOpen ? FindQuote(pOpen) : NULL; char* q2 = q1 ? FindQuote(q1 + 1) : NULL;
                if (pOpen) {
                    char* comma1 = strchr(next, ',');
                    if (comma1 && (!q1 || comma1 < q1)) {
                        s->ptsX[0] = atof(pOpen + 1); s->ptsY[0] = atof(comma1 + 1);
...
int LoadCFile(const char* path) {
    FILE* f; long sz; char *d, *cur, *endBlock, *st, *pt, *lStr, *ext, *tag, *dim, *pgDef, *next;
    Shape** tmpShapesArr; int i, cId = 1, tmpCount = 0;
    static Dimension tmpDims[MAX_DIMS]; int tmpDimCount = 0;
    char tmpPageName[64] = "A4 (210x297)"; char tmpUnitName[32] = "mm"; double tmpPageScale = 1.0;

    FreeAllData();
    tmpShapesArr = (Shape**)calloc(MAX_SHAPES, sizeof(Shape*));
    if (!tmpShapesArr) return 0;

    strcpy(loadedCFile, path);
    f = fopen(path, "rb"); if (!f) { loadedCFile[0] = '\0'; free(tmpShapesArr); return 0; }
    fseek(f, 0, SEEK_END); sz = ftell(f); fseek(f, 0, SEEK_SET);
    if (sz <= 0 || sz > 60000L) { fclose(f); free(tmpShapesArr); return 0; }

    d = (char*)calloc(1, (size_t)sz + 1);
    if (!d) { fclose(f); free(tmpShapesArr); return 0; }
    fread(d, 1, (size_t)sz, f); d[sz] = '\0'; fclose(f);
    ...
    while ((cur = strstr(cur, "case ")) != NULL && parsedCount < MAX_ICONS) {
        ...
        while (st < endBlock && tmpCount < MAX_SHAPES) {
            ...
            dim = strstr(st, "DIMENSION("); pgDef = strstr(st, "PAGE_DEF(");
            ...
            if (next == pgDef) {
                ...
                if (q_cnt >= 6) {
                    ...
                    char* comma = strchr(q[5] + 1, ','); if (comma) tmpPageScale = atof(comma + 1);
                } else if (q_cnt >= 4) {
                    ...
                    char* comma = strchr(q[3] + 1, ','); if (comma) tmpPageScale = atof(comma + 1);
                }
                st = strchr(next, ';'); if (!st) st = next + 1; continue;
            }
            ...
            if (next == dim) {
                ...
                if (pOpen) {
                    ...
                    if (c1 && c2 && c3 && c4 && c5 && c6 && tmpDimCount < MAX_DIMS) {
                        tmpDims[tmpDimCount].s1 = atoi(pOpen + 1); tmpDims[tmpDimCount].p1 = atoi(c1 + 1);
                        tmpDims[tmpDimCount].s2 = atoi(c2 + 1); tmpDims[tmpDimCount].p2 = atoi(c3 + 1);
                        tmpDims[tmpDimCount].offset = atof(c4 + 1); tmpDims[tmpDimCount].textPos = atof(c5 + 1);
                        tmpDims[tmpDimCount].mode = atoi(c6 + 1); tmpDimCount++;
                    }
                }
                st = strchr(next, ';'); if (!st) st = next + 1; continue;
            }
            ...
            if (!tmpShapesArr[tmpCount]) {
                tmpShapesArr[tmpCount] = (Shape*)calloc(1, sizeof(Shape));
            }
        ...
        if (parsedIcons[parsedCount]) {
            ...
            if (tmpDimCount > 0) {
                memcpy(parsedIcons[parsedCount]->dims, tmpDims, tmpDimCount * sizeof(Dimension));
            }
            parsedCount++;
        }
        cur = endBlock;
    ...
    for (i = 0; i < MAX_SHAPES; i++) {
        if (tmpShapesArr[i]) free(tmpShapesArr[i]);
    }
    free(tmpShapesArr);
...
void ExportCtoDXF(const char* c_path, const char* save_path, double scale) {
    ...
    for (i = 0; i < parsedCount; i++) {
        ...
        if (parsedIcons[i]->dimCount > 0) {
            DrawDXFDimensions(f, parsedIcons[i]->dims, parsedIcons[i]->dimCount, parsedIcons[i]->shapes, parsedIcons[i]->shapeCount, scale, 0.0, 0.0, 0.0, 0.0, parsedIcons[i]->unitName);
        }
                ...
                if (strcmp(val, "VERTEX") == 0 || strcmp(val, "SEQEND") == 0) {
                    ...
                    if (strcmp(entType, "LINE") == 0 && s.ptCount == 2) {
                        ...
                        if (tempShapeCount < MAX_SHAPES) {
                            tempShapes[tempShapeCount] = (Shape*)calloc(1, sizeof(Shape));
...
void CalcBoundingBox(Shape** shapes, int count, Dimension* dims, int dimCount, double* outMinX, double* outMinY, double* outMaxX, double* outMaxY) {
    int i, p; double minX = 99999.0, minY = 99999.0, maxX = -99999.0, maxY = -99999.0;
    for (i=0; i<count; i++) {
        Shape* sh = shapes[i];
        if (sh->type == 3) {
            char* refPath = (char*)calloc(1, 512);
            char* absPath = (char*)calloc(1, 512);
                ...
                if (refPath[0]) {
                    ...
                    if (!loadedCFile[0] || _stricmp(absPath, loadedCFile) != 0) {
                        ...
                        if (rIdx != -1) {
                            ...
                            minX = fmin(minX, sh->ptsX[0]); maxX = fmax(maxX, sh->ptsX[0] + 10.0); 
                            minY = fmin(minY, sh->ptsY[0]); maxY = fmax(maxY, sh->ptsY[0] + 5.0);
                        }
                    }
                }
            }
...
void DrawSVGShape(FILE* f, Shape* sh, double scale, double minX, double minY, double iconPageScale) {
    ...
    if (sh->type == 3) {
        ...
        if (rIdx != -1) {
            ...
            if (refCache[rIdx]->dimCount > 0) {
                DrawSVGDimensions(f, refCache[rIdx]->dims, refCache[rIdx]->dimCount, refCache[rIdx]->shapes, refCache[rIdx]->shapeCount, scale, 0.0, 0.0, minX, minY, refCache[rIdx]->unitName);
            }
        } else {
            double tx = (sh->ptsX[0] - minX) * scale;
            double ty = (sh->ptsY[0] - minY) * scale;
            fprintf(f, "<rect x=\"%.2f\" y=\"%.2f\" width=\"20.0\" height=\"10.0\" fill=\"none\" stroke=\"#FF0000\" stroke-dasharray=\"2,2\" />\n", tx, ty);
            fprintf(f, "<text x=\"%.2f\" y=\"%.2f\" font-family=\"Arial\" font-size=\"4.0\" fill=\"#FF0000\">Missing: %s</text>\n", tx + 1.0, ty + 6.0, refPath);
        }
        return;
    }
...
void ExportCtoSVG(const char* c_path, const char* save_path, double scale) {
    ...
    for (i=0; i<parsedCount; i++) {
        ...
        if (parsedIcons[i]->dimCount > 0) {
            DrawSVGDimensions(f, parsedIcons[i]->dims, parsedIcons[i]->dimCount, parsedIcons[i]->shapes, parsedIcons[i]->shapeCount, scale, 0.0, 0.0, minX, minY, parsedIcons[i]->unitName);
        }
...
void ImportAction(HWND hwnd, int isDXF) {
    ...
    if (strlen(out_path) == 0) {
        ...
        if (strchr(out_path, '\\') || strchr(out_path, '/') || (out_path[0] && out_path[1] == ':')) {
    ...
    scale *= 10.0; /* Make 1.0 default equal 10X scale internally */
